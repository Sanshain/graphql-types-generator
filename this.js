	 ...
	 */
	 getTypes(filename, codeTypes, graTypes) {
		...
		for (const query of gqls) {
			...
			// @ts-ignore
			let definition = gql$1(query).definitions.pop();
...
const { TypesGenerator: TypesGenerator$1 } = utils;
// let filename = 'queries.js';

var main = async function typesGenerate(options) {
	...
	const generator = new TypesGenerator$1(options);
	...
	fs__default['default'].writeFile(path__default['default'].join(__dirname, target), codeTypes, () => {
